...
const styles = {
  dialog: {
    content: "max-w-[800px] w-[95vw] max-h-[85vh] overflow-y-auto p-4 md:p-6",
  ...
  stepper: {
    ...
    container: "flex justify-between items-start relative gap-2 md:gap-4",
    step: "flex flex-col items-center relative z-10 flex-1 px-1 md:px-2",
    icon: "w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 mb-2 shadow-sm",
    ...
    label: "text-xs font-medium text-center max-w-[80px] leading-tight mt-1",
...
export function EditPetDialog({ pet, open, onOpenChange, onSuccess }: EditPetDialogProps) {
  ...
  useEffect(() => {
    if (open && pet) {
      setPetData({
        profile_image: null,
        current_image_url: pet.profile_image_url,
        name: pet.name || '',
        type: pet.type || '',
        breed: pet.breed || '',
        gender: pet.gender || '',
        ...
        medical_conditions: pet.medical_info?.conditions?.join(', ') || '',
        allergies: pet.medical_info?.allergies?.join(', ') || '',
        medications: pet.medical_info?.medications?.join(', ') || '',
        vaccinations: pet.medical_info?.vaccinations || [],
        veterinarian_name: pet.medical_info?.veterinarian?.name || '',
        veterinarian_phone: pet.medical_info?.veterinarian?.phone || '',
        veterinarian_clinic: pet.medical_info?.veterinarian?.clinic || '',
        emergency_contact_name: pet.emergency_contact?.name || '',
        emergency_contact_relationship: pet.emergency_contact?.relationship || '',
        emergency_contact_phone: pet.emergency_contact?.phone || '',
      });
      setCurrentStep(1);
    }
  }, [pet, open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let profile_image_url = petData.current_image_url;
      if (petData.profile_image) {
        const fileExt = petData.profile_image.name.split('.').pop();
        const fileName = `${pet.id}.${fileExt}`;
      ...
      const petUpdateData = {
        ...
        medical_info: {
          ...
          veterinarian: {
            name: petData.veterinarian_name || null,
            phone: petData.veterinarian_phone || null,
            clinic: petData.veterinarian_clinic || null,
            email: pet.medical_info?.veterinarian?.email || null,
            address: pet.medical_info?.veterinarian?.address || null,
            specialization: pet.medical_info?.veterinarian?.specialization || null,
            license_number: pet.medical_info?.veterinarian?.license_number || null,
          }
        },
        emergency_contact: {
          name: petData.emergency_contact_name || null,
          relationship: petData.emergency_contact_relationship || null,
          phone: petData.emergency_contact_phone || null,
          email: pet.emergency_contact?.email || null,
          address: pet.emergency_contact?.address || null,
        }
      };
      ...
      toast({
        title: t('dashboard.pets.edit.success.title'),
        description: t('dashboard.pets.edit.success.description', { name: petData.name }),
      });
      ...
      toast({
        title: t('dashboard.pets.edit.error.title'),
        description: t('dashboard.pets.edit.error.description'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
                  ...
                  <div className={styles.layout.formGroup}>
                    ...
                    <div className={styles.uploader.container}>
                      <label className={styles.uploader.dropzone}>
                        ...
                        ) : petData.current_image_url ? (
                          <>
                            <img
                              src={petData.current_image_url}
                      ...
                      <p className="text-xs text-muted-foreground text-center mt-3">
                        {petData.current_image_url ? 'Click to change photo' : 'Optional: Add a profile photo for your pet'}
                      </p>
                    </div>
                  ...
                  <div className={styles.footer.buttonGroup}>
                    ...
                    {currentStep < steps.length ? (
                      ...
                      <Button type="submit" disabled={loading}>
                        ...
                        {t('dashboard.pets.edit.saveChanges')}
                      </Button>
                    )}
                  </div>
                </DialogFooter>
