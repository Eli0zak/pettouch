...
const AdminLostFound = () => {
  ...
  const fetchInteractions = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        `)
        .in('interaction_type', ['found', 'know'])
        .order('created_at', { ascending: false });
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to fetch interactions",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const filteredInteractions = interactions.filter(interaction => {
    const searchLower = searchQuery.toLowerCase();
    const matchesSearch = !searchQuery || 
      (interaction.post?.title && interaction.post.title.toLowerCase().includes(searchLower)) ||
      (interaction.post?.description && interaction.post.description.toLowerCase().includes(searchLower)) ||
      (interaction.name && interaction.name.toLowerCase().includes(searchLower)) ||
      (interaction.email && interaction.email.toLowerCase().includes(searchLower)) ||
      (interaction.phone && interaction.phone.toLowerCase().includes(searchLower)) ||
      (interaction.message && interaction.message.toLowerCase().includes(searchLower));
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => { fetchReports(); fetchInteractions(); }}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <Input
            placeholder="Search reports and interactions..."
            className="pl-10"
            value={searchQuery}
      ...
      <Card>
        {loading ? (
          ...
          <>
            <h2 className="text-2xl font-semibold mb-4">Lost & Found Posts</h2>
            <Table>
              <TableHeader>
                <TableRow>
            ...
            <h2 className="text-2xl font-semibold mt-10 mb-4">Lost & Found Interactions</h2>
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {filteredInteractions.length === 0 ? (
                  ...
                  filteredInteractions.map((interaction) => (
                    <TableRow key={interaction.id}>
                      <TableCell>{interaction.post?.title || 'N/A'}</TableCell>
                      <TableCell>{interaction.name}</TableCell>
                      <TableCell>{interaction.email}</TableCell>
                      <TableCell>{interaction.phone || 'N/A'}</TableCell>
